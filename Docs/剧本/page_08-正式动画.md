...
| --- | --- | --- | --- | --- |
| 1 | `0.00s - 1.50s` | 引入 UE PSO 分层。 | PSO/Hash 结构 | 强调哈希与状态。 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `pso-node` | UE PSO 主节点。 | 承接 page7 | 主节点 | 成立。 | `7.2/10` | 可读。 | `7.2/10` |
| `hash-node` | VS/PS Hash 关联节点。 | 承接 page7 lookup 语义 | 主节点 | 成立。 | `7.1/10` | 可读。 | `7.1/10` |

## 2026-04-21 文案微调复核

- 新增元素：无。
- 移动节点：右上角 `proof material` 组整体放大，并向外扩了现有容器盒与内部 `Cooked ShaderCode` cue；其余节点不动。
- 增加 / 删除 node / edge：无新增、无删除。
- 行为替换：无动画行为替换；在既有 proof 组内把标签 `Material` 改为 `Material A`，并将容器底色改为沿用 `uasset` 的绿色系。
