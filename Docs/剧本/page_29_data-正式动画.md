...
| 指标 | 分数 | 说明 |
| --- | --- | --- |
| 布局分数 | `整体布局 Review=3.7/10` | 代码区与平台表都更大更低了，但 formal mechanical review 仍提示局部口袋偏紧，尤其是底部外边距。 |
| 模块分数 | `模块空间结构 Review=4.3/10` | 上下阅读顺序已经稳定，重点提示也回收到各自卡片右上角，不过整体仍偏紧。 |
...
| 页面标题 | `无（移除页内主标题）` |
| 页面副标题 | `无（移除页内副标题）` |
| 当前页核心信息 | 顶排直接由 `Vertex Shader / Fragment Shader` 占住左上和右上，而且标题与代码比上一版至少再大两档；shader 代码行继续保留真实缩进并加轻量语法高亮；原本右上角那组开关下移成第二排左右各一块，也去掉 `State 开关` 可见组标题、外层框与 API 对照标签；两块 state 现在直接把结论写进顶行，不再保留灰色解释行；下方保留轻表格化的 PC / Android 极值表，但重点提示被收到各自 node 的右上角，PC 的 `VK off` 两个低耗时值会标成绿色，明显的高耗时值统一改成橙色，`loop=5000` 行名保持黑色，同时整张表在卡内继续下压。 |
...
| 这一页移动了什么 | 顶排维持 `vertex-code` 左上、`fragment-code` 右上，但标题与代码继续放大；`state-vk / state-gl` 维持第二排左右各一块，同时把原第三行灰色说明折叠掉，只保留顶行结论 + 第二行代码；`pc-card / android-card` 里的重点提示被挪到右上角，表格本体也继续往下压，减少卡底空白。 |
...
| `pc-card` | `PC（RTX 3080）` | 桌面端极值表容器 | rounded card | 主舞台左中 | 高 | 与 `android-card` 同宽同高；右上角重点提示直接收成 `VK off 0.0653ms -> 0.0645ms`，两组低耗时值改绿，而 `59.1658 / 32.0594` 这类高耗时值改橙。 | 承接上一页的平台差异证据 | 否 |
...
| `android-card` | `Android（Adreno）` | 移动端极值表容器 | rounded card | 主舞台右中 | 高 | 与 `pc-card` 同宽同高；右上角重点提示收成一句 `loop=5000` 三列贴近 `400ms`，峰值三列高耗时统一改橙。 | 承接上一页的平台差异证据 | 否 |
...
- 双平台重点提示都要挂在各自 card 的右上角；PC 卡里绿色只留给 `VK off` 的两个低耗时值，明显高耗时值改橙，`loop=5000` 行名保持黑色。
...
- 不再保留 `API 对照` 小标签位。
- 重点提示并入标题区，不再额外长出独立的 focus strip；右上角提示里保留绿色低耗时，同时把明显高耗时值统一收成橙色。
...
| 移动的节点 | `vertex-code / fragment-code` 维持左右上角两块，但标题与代码继续放大；`state-vk / state-gl` 维持第二排左右各一块，并把灰色说明折叠进顶行结论；`pc-card / android-card` 的重点提示挪到右上角，统计表继续下压成横向吃满外层 node 宽度、顶部直线分隔的轻表格 |
...
| 风险点 | 顶部 shader 区字号已经继续放大，底部外边距也比上一版更紧；如果后面还要继续压大字号或再把表格往下放，优先会撞到底部安全边距。 |
...
- 复核结论：本轮先按几何约束和机械 review 调整，重点是代码字号继续放大、平台卡右上角提示回收、表格继续下压；若还需要进一步挑视觉，再补新 capture 做二次读图复核。

## 三层 Review 总表

| Review 层级 | Art Critic | Geometry Critic | 综合判断 | 综合分数 |
| --- | --- | --- | --- | --- |
| `整体布局 Review` | `formal harness` | `3.7/10` | 顶排与底排语义已经收顺，但底部外边距进一步收紧。 | `通过` |
...
| `pc-card` | 桌面端极值表，重点压 `VK off` 稳定性。 | 从前提落到桌面端结果 | rounded card | 成立。 | `通过` | 重点提示挪到右上角后，绿色只留给两个低耗时值，橙色留给高耗时对照。 | `通过` |
| `android-card` | 移动端极值表，对照说明优化没有兑现。 | 从前提落到移动端结果 | rounded card | 成立。 | `通过` | 右上角提示把结论先收短，峰值三列的橙色让高耗时更一眼可见。 | `通过` |
