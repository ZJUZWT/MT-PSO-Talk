// ====================================================================
// vulkan_test_android.cpp – Vulkan PSO benchmark on Android
//
// Same logic as the desktop version but adapted for Android:
//   - No GLFW, uses Vulkan directly (offscreen, no surface needed)
//   - Loads SPIR-V from Android assets
//   - Uses vkCmdWriteTimestamp for GPU timing
// ====================================================================
...
static void createInstance(VulkanCtx& ctx);
static void pickPhysicalDevice(VulkanCtx& ctx);
static void createDevice(VulkanCtx& ctx);
static void createCommandPool(VulkanCtx& ctx);
static void createRenderTarget(VulkanCtx& ctx, uint32_t w, uint32_t h);
static void createRenderPass(VulkanCtx& ctx);
static void createFramebuffer(VulkanCtx& ctx, uint32_t w, uint32_t h);
static void createPipeline(VulkanCtx& ctx, AAssetManager* assetMgr);
static void createBaselinePipeline(VulkanCtx& ctx, AAssetManager* assetMgr);
static void createVertexBuffer(VulkanCtx& ctx, const std::vector<float>& vertices);
static void createQueryPool(VulkanCtx& ctx);
static void cleanup(VulkanCtx& ctx);
static uint32_t findMemoryType(VulkanCtx& ctx, uint32_t filter, VkMemoryPropertyFlags props);

// ====================================================================
// Public entry point
// ====================================================================
BenchmarkResult runVulkanBenchmark(int iterations, AAssetManager* assetMgr, int loopCount) {
    ...
    try {
        ...
        LOGI("Vulkan GPU time: %.4f ms", result.gpuTimeMs);
...
static void createInstance(VulkanCtx& ctx) {
    ...
    // No extensions needed for offscreen rendering on Android
...
static void createDevice(VulkanCtx& ctx) {
    ...
    for (uint32_t i = 0; i < qfCount; i++) {
        if ((qfProps[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) &&
            qfProps[i].timestampValidBits > 0) {
            ctx.queueFamily = i;
            found = true;
            break;
        }
    ...
    if (!found) {
        // Fallback: just find any graphics queue
        for (uint32_t i = 0; i < qfCount; i++) {
            if (qfProps[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) {
                ctx.queueFamily = i;
                found = true;
...
static void createPipeline(VulkanCtx& ctx, AAssetManager* assetMgr) {
    // ── Load SPIR-V from assets ────────────────────────────────────
    auto vertCode = readAsset(assetMgr, "shaders/spirv/heavy.vert.spv");
    auto fragCode = readAsset(assetMgr, "shaders/spirv/heavy.frag.spv");
    ...
    LOGI("Pipeline created with colorWriteMask = 0");
}
...
static void createColorWritePipeline(VulkanCtx& ctx, AAssetManager* assetMgr) {
    auto vertCode = readAsset(assetMgr, "shaders/spirv/heavy.vert.spv");
    auto fragCode = readAsset(assetMgr, "shaders/spirv/heavy.frag.spv");
    ...
    LOGI("Pipeline created with colorWriteMask = RGBA (full write)");
}

// ====================================================================
// Heavy shader + colorWriteMask ENABLED benchmark
// ====================================================================
BenchmarkResult runVulkanBenchmarkColorWrite(int iterations, AAssetManager* assetMgr, int loopCount) {
    BenchmarkResult result;
    result.backend    = "Vulkan";
    ...
    try {
        ...
        createRenderPass(ctx);
        createFramebuffer(ctx, W, H);
        createColorWritePipeline(ctx, assetMgr);
        createVertexBuffer(ctx, gridVertices);
        createQueryPool(ctx);
        ...
        LOGI("Vulkan ColorWrite GPU time: %.4f ms", result.gpuTimeMs);
...
// ====================================================================
static void createBaselinePipeline(VulkanCtx& ctx, AAssetManager* assetMgr) {
    auto vertCode = readAsset(assetMgr, "shaders/spirv/baseline.vert.spv");
    auto fragCode = readAsset(assetMgr, "shaders/spirv/baseline.frag.spv");
    ...
    LOGI("Baseline pipeline created (empty shader, normal colorWriteMask)");
}
...
BenchmarkResult runVulkanBaseline(int iterations, AAssetManager* assetMgr) {
    ...
    try {
        ...
        createRenderPass(ctx);
        createFramebuffer(ctx, W, H);
        createBaselinePipeline(ctx, assetMgr);
        createVertexBuffer(ctx, gridVertices);
        createQueryPool(ctx);
        ...
        LOGI("Vulkan Baseline GPU time: %.4f ms", result.gpuTimeMs);
